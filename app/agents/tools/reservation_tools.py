...
@tool
async def ver_mesas_disponibles(nombre_evento: str, tipo_mesa: str = None) -> str:
    """
    ...
    try:
        ...
        logger.error(f"ver_mesas_disponibles error: {exc}")
        return _json({"error": str(exc)})


@tool
async def reservar_mesa(telegram_id: int, nombre_evento: str, numero_mesa: int) -> str:
    ...
    try:
        async with get_connection() as conn:  # noqa: SIM117
            async with conn.transaction():
                ...
                if not mesa:
                    return _json({"status": "error", "message": "La mesa no está disponible."})
...
@tool
async def cancelar_mi_reserva(telegram_id: int, numero_mesa: int) -> str:
    ...
    try:
        async with get_connection() as conn:  # noqa: SIM117
            async with conn.transaction():
                ...
                return _json({"status": "success", "message": "Reserva cancelada exitosamente."})
    except Exception as exc:
        logger.error(f"cancelar_mi_reserva error: {exc}")
        return _json({"error": str(exc)})
